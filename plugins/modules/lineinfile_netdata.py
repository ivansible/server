...
description:
  ...
  - This special version of the module honors I(regexp), I(backrefs), and I(insertafter) together,
    I(insertbefore) is not supported.
version_added: "0.7"
options:
  path:
    description:
  ...
  regexp:
    description:
      ...
      - If the regular expression is not matched, the module will fail.
  ...
  backrefs:
    description:
      ...
      - The last line matching line C(regexp) will be replaced by the expanded line parameter.
    type: bool
    default: no
    version_added: "1.1"
  insertafter:
    description:
      - Used with C(state=present).
      - The line will be inserted after the first match of specified regular expression.
      - If the last match is required, use(firstmatch=no).
      - The specified regular expression must match, or module will fail.
    type: str
    required: true
  create:
    description:
  ...
  firstmatch:
    description:
      - Used with C(insertafter).
      - If set, C(insertafter) will work with the first line that matches the given regular expression.
    type: bool
    default: yes
    version_added: "2.5"
  others:
    ...
    if validate:
        ...
        if rc != 0:
            module.fail_json(msg='failed to validate: '
                                 'rc:%s error:%s' % (rc, err))
    if valid:
        b_realpath = os.path.realpath(to_bytes(dest, errors='surrogate_or_strict'))
        module.atomic_move(tmpfile,
                           to_native(b_realpath, errors='surrogate_or_strict'),
...
def present(module, dest, regexp, line, insertafter, create,
    ...
    # index[1] is the line num where insertafter has been found
    index = [-1, -1]
    match = None
    b_line = to_bytes(line, errors='surrogate_or_strict')

    # 1. Parse for searching insertafter:
    for lineno, b_cur_line in enumerate(b_lines):
        if bre_ins.search(b_cur_line):
            index[1] = lineno + 1
            if firstmatch:
                break
    if index[1] == -1:
        module.fail_json(msg='insertafter not found')
    # `insertafter` matched a line in the file

    # 2. Check that there is a match for regexp:
    for lineno, b_cur_line in enumerate(b_lines):
        if lineno < index[1]:
            continue
        ...
        if match_found:
            ...
            if firstmatch:
                break
    if index[0] == -1:
        module.fail_json(msg='regexp not found')
...
def main():
    ...
    if regexp in ['', '^', '$']:
        module.fail_json(msg='invalid regexp value')
    if insertafter in ['', '^', '$', 'BOF', 'EOF']:
        module.fail_json(msg='invalid insertafter value')
    ...
    if params['state'] == 'present':
        ...
        present(module, path, regexp, line, insertafter, create, backup, backrefs, firstmatch)
    else:
